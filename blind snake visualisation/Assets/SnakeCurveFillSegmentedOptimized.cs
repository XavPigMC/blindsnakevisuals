...
public class SnakeCurveFillSegmentedOptimized : SnakeCurveFillSegmented 
{
    public SnakeCurveFillSegmentedOptimized(Texture2D texture, Vector2Int applePosition) : base(texture, applePosition)
    {
    ...
    public bool DO_IT_SURFACE_OPTIMIZED(uint currentPowerOfTwo)
    {
        ...
        if (currentPowerOfTwo % 2 == 0)
        {
            ...
            if((snakePosition.y + 1) * (snakePosition.x-1) > previousSurfaceArea && snakePosition.x-1 > 2) //gotta catch em all
                if (LEFT())
                    return true;
        ...
        else
        {
            ...
            if (goingRight && (snakePosition.x+1)*snakePosition.y <= currentSurfaceArea)//means it's been going left in the last cycle
